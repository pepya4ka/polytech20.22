...
#ifndef _foo_m_temp1_H_
#define _foo_m_temp1_H_

#include <systemc>
#include <iostream>
#include <fstream>

#include "foo_m_temp1_memcore.h"

SC_MODULE(foo_m_temp1) {
...
protected:
    ...
    foo_m_temp1_memcore* foo_m_temp1_memcore_U;
...
public:
    ~foo_m_temp1();
    SC_CTOR(foo_m_temp1) : m_trace_file(0) {
        SC_METHOD(proc_memcore_addr);
        sensitive << i_address0 << t_address0 << iptr << tptr;

        foo_m_temp1_memcore_U = new foo_m_temp1_memcore("foo_m_temp1_memcore_U");
        foo_m_temp1_memcore_U->clk      ( clk );
        foo_m_temp1_memcore_U->reset    ( reset );
        foo_m_temp1_memcore_U->ce0      ( i_ce0 );
        foo_m_temp1_memcore_U->we0      ( i_we0 );
        foo_m_temp1_memcore_U->address0 ( memcore_iaddr );
        foo_m_temp1_memcore_U->d0       ( i_d0 );
        foo_m_temp1_memcore_U->q0       ( i_q0 );

        foo_m_temp1_memcore_U->ce1      ( t_ce0 );
        foo_m_temp1_memcore_U->we1      ( t_we0 );
        foo_m_temp1_memcore_U->address1 ( memcore_taddr );
        foo_m_temp1_memcore_U->d1       ( t_d0 );
        foo_m_temp1_memcore_U->q1       ( t_q0 );
        ...
        if (dump_vcd) {
            m_trace_file = sc_create_vcd_trace_file( "foo_m_temp1.trace");
            // ports
